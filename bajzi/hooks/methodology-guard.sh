...
case "$choice" in
  gsd)
    emit "" "MÓDSZERTAN ebben a repóban: GSD. A tervezés/végrehajtás/review/debug munkát a gsd-* skillekkel végezd (/gsd-plan-phase, /gsd-execute-phase, /gsd-code-review, /gsd-debug). A superpowers azonos célú skilljeit (brainstorming, writing-plans, executing-plans, systematic-debugging, requesting-code-review) NE használd itt — a superpowers többi, nem ütköző skillje (using-git-worktrees, dispatching-parallel-agents, test-driven-development) használható. Ezt a felhasználó egyszer eldöntötte, ne kérdezd újra."
    ;;
  superpowers|sp)
    emit "" "MÓDSZERTAN ebben a repóban: superpowers. A tervezés/végrehajtás/review/debug munkát a superpowers skilljeivel végezd (brainstorming, writing-plans, executing-plans, subagent-driven-development, systematic-debugging, requesting-code-review, verification-before-completion). A gsd-* skilleket NE használd itt, és ne hozz létre .planning/ struktúrát. Ezt a felhasználó egyszer eldöntötte, ne kérdezd újra."
    ;;
  none|nincs)
    emit "" "MÓDSZERTAN ebben a repóban: egyik sem. Ne indíts se GSD-, se superpowers-folyamatot magadtól; dolgozz közvetlenül, ahogy a feladat kívánja. Ezt a felhasználó egyszer eldöntötte, ne kérdezd újra."
    ;;
  *)
    emit "Ebben a repóban még nincs kiválasztva a vezető módszertan (GSD vagy superpowers). Az első érdemi feladatnál Claude egyszer rá fog kérdezni — vagy futtasd: /bajzi:modszertan" \
         "FIGYELEM: ebben a repóban nincs .claude/METHODOLOGY jelölőfájl, és a GSD meg a superpowers is telepítve lehet. A kettő ugyanazt a területet fedi, ezért AZ ELSŐ olyan feladatnál, ami tervezést, több lépéses végrehajtást, code review-t vagy szisztematikus debugot igényel, EGYSZER kérdezd meg a felhasználót, melyik legyen a vezető ebben a repóban (gsd / superpowers / egyik sem), majd írd a válaszát egyetlen szóként a .claude/METHODOLOGY fájlba. Apró, egylépéses feladatnál ne kérdezz — csak csináld meg."
    ;;
esac
