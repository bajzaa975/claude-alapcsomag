...
1. `FC check` — exit 1 = DEBT CAP HIT: print its output and STOP. The slice does not start until
   `/bajzi:debt --drain` brings the debt under the cap (D6).
2. `FC slice <slice-id>` — prints `agent:`, `tier:`, `files:`, `test:`. Exit 2 = a bad slice file
   (`docs/slice-format.md`): print the error, STOP. The agent is this lookup, never your choice.
...
7. Print `range: <base>..<new HEAD>` and the next step: `/bajzi:review <slice-id> <base>..<HEAD>`.
