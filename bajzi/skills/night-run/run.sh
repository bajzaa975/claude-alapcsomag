...
End the story with the review-and-fix loop: a FRESH, INDEPENDENT Opus 5.5 sub-agent reviews the diff against origin/$BASE_BRANCH (never your summary of it) and returns VERDICT pass|fail with BLOCKING / NON-BLOCKING findings and an EVIDENCE line; a different sub-agent fixes blocking findings; repeat with a new reviewer, at most 3 rounds. Zero blocking findings = review-green. Still blocking after round 3, or the same finding twice with no change in the diff -> PARK with reason review. Never reach green by deleting or skipping a test, loosening an assertion or lowering a threshold.
