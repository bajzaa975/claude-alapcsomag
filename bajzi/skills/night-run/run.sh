...
End the story with the review-and-fix loop: a FRESH, INDEPENDENT sub-agent on the reviewer model BRIEF section 4 names (the reviewer allow-list) reviews the diff against origin/$BASE_BRANCH (never your summary of it) and returns VERDICT pass|fail with BLOCKING / NON-BLOCKING findings and an EVIDENCE line; a different sub-agent fixes blocking findings; repeat with a new reviewer, at most 2 rounds (round 1 reviews the diff, round 2 verifies the fixes). Zero blocking findings = review-green. Still blocking after round 2, or the same finding twice with no change in the diff -> PARK with reason review; the third round is the owner's call, never yours, and never taken by rewriting a refused brief by hand. Never reach green by deleting or skipping a test, loosening an assertion or lowering a threshold.
Whatever your LAST action depends on runs in the FOREGROUND: the final gate and any command whose result you must act on before ending the turn are run without run_in_background; long intermediate runs may still go to the background. A headless session that ends its turn to wait for a background task exits right there, the task is killed, and the RESULT line below is never written (six sprints were lost this way on 2026-09-25).
...
RESULT $1 <merged|open|parked|blocked> PR#<number or -> reason=<review|timeout|context|ci|forbidden|tests|merge-forbidden|->
Use reason=- only when the story is merged with nothing to flag; forbidden = a forbidden-zone rule stopped you, tests = a test failure you could not fix, ci = CI was red or never went green, context = you hit your context floor, timeout = you ran out of wall clock, review = the review loop never went green, merge-forbidden = the PR is green but the project's rules do not allow the night session to merge; the PR stays open for the morning sweep.
