...
check "$(grep -qE '^L1 ' "$lnd"/state-*.txt 2>/dev/null && echo 1 || echo 0)" "(iv) …and the skip recorded no state row"
...
leaks=0; seen=0
for f in "$ROOT"/t*/nd/fake/fds.log "$nd/fake/fds.log" "$ROOT"/live/nd/fake/fds.log; do
  [ -s "$f" ] || continue
  seen=$((seen + 1))
