...
1. `FC copy fixer <findings-file>` — writes `<slice-id>-r1.fixer.md` (no severity, D4).
   Exit 3 = STOP: a round-2 file is past the cap; print the message, do nothing else.
   `CLEAN: nothing to fix` -> done.
2. `FC slice <slice-id>` for `files:` and `test:`. Remember `base` = the left side of the r1
   file's `range:` line.
3. Dispatch per dispatch.md, class `fix`, brief = the fixer brief on the `.fixer.md` file.
   Save its final message to `runtime/findings/<slice-id>-r1.report.md`.
4. Gate: `.githooks/pre-commit` if the repo has one, else the `test:` command. Red -> STOP and
   report; no commit, no round 2.
5. A path in `git status --porcelain` outside `files:` (ignore `runtime/**`, which the skills
   write) -> STOP and report. Else `git add -- <each changed file>` (never `-A`), `git commit -m "<slice-id>: fix r1 findings"`.
6. Run `/bajzi:review <slice-id> <base>..HEAD --round 2 --previous <findings-file>`.
