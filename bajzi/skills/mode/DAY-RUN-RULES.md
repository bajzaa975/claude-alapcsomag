...
- review a diff -> OPUS 5.5, ALWAYS. Never ORCH's model, never the implementer's model, never a
  cheaper tier because the diff "looks small". Fresh context every round; EVIDENCE line required
  or the verdict is FAIL.
- final whole-branch review -> OPUS 5.5, ALWAYS, on every development without exception. It does not
...
Each task ends with its own Opus 5.5 review giving TWO verdicts, spec compliance and quality. When
all tasks are done the branch gets a final Opus 5.5 whole-branch review. Findings go to a FIXER
sub-agent -- never the reviewer that raised them, never ORCH -- and the fix gets a NEW Opus 5.5
...
limit reached. Restart with Opus." Next session: claude --model claude-opus-5-5. Same rules reload;
