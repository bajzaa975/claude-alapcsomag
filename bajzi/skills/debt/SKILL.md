...
**--check** — `FC check`. Exit 1 = `DEBT CAP HIT` (> 15 entries, or > 3 in one file, or an
...
4. Dispatch class `drain-review` over `<base>..HEAD` with `debt.md` as the round-1 file.
   Save to `runtime/findings/debt-r2.md`, `FC validate` it (one retry, as /bajzi:review step 3).
...
2. Dispatch class `calibrate` (brief: `Calibration mode.` + that file). Save the reply to
   `runtime/findings/debt.rerate.txt`.
3. `FC calibrate runtime/findings/debt.rerate.txt` — writes `blind_severity` into `debt.md` and
   ...
   your call`). Exit 2 (an id not re-rated) -> re-dispatch once, then STOP.
