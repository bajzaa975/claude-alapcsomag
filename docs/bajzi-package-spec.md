...
| Review/fix loop skills (`/bajzi:implement`, `/bajzi:review`, `/bajzi:fix`, `/bajzi:debt`): round cap, D4 routing to `needs-owner.md`/`debt.md`, D6 cap check, D7 calibrate, dispatch log | `bajzi/skills/{implement,review,fix,debt}/SKILL.md` (the procedure), `bajzi/skills/lib/dispatch.md` (brief templates, dispatch table, log step), `bajzi/lib/findings-cli.js` `cmds.slice`/`cmds.validate`/`cmds.copy`/`cmds.close`/`cmds.check`/`cmds.drain`/`cmds.calibrate`/`cmds.log`, `toOwner` (the `needs-owner.md` renderer) | `timeout 300 bash bajzi/skills/mode/tests/mode.sh </dev/null` (case 16) | 1 | Every decision (which agent, which round, where an open finding goes, cap hit) is made by `findings-cli.js` on files, never by the model; a SKILL.md only says when to call it. The round cap is enforced twice: `copy fixer` on a round-2 file and `validate` on a `-r3.md` file both exit 3 (`STOP`). `close` needs `<slice>-r1.md`, `-r2.md` and `-r1.report.md` (the fixer's final message, saved by `/bajzi:fix`). The skills' `SKILL-<CLASS>` log lines sit beside the dispatch guard's own R4 lines when its gate is open; count one or the other, not both. |
...
| day-run / saver / dispatch-guard / review-loop bash suite | `timeout 300 bash bajzi/skills/mode/tests/mode.sh </dev/null` (about 2 min) | `bajzi-plugins-dev` | Git Bash; the `timeout` + `</dev/null` avoid a hang on a case that reads stdin |
...
13. **The fixer never sees severity, and the reviewer never writes.** `fixer` is dispatched on a
    ...
    `Agent`/`Task` (`agents.test.js` `PINS`, `validateAgent`). The skills own the two-round cap:
    every review runs through `reviewer` and every fix through `fixer` via `/bajzi:review` and
    `/bajzi:fix`, which stop after round 2 (`findings-cli.js` `cmds.copy`/`cmds.validate` exit 3).
...
decision, and dispatches through the shared template `bajzi/skills/lib/dispatch.md`: write the
brief to `runtime/briefs/<slice>-<class>.txt` with the evidence inline, dispatch
`bajzi:<agent>` with that text, log one line (`findings-cli.js log`), save the agent's final
message verbatim.

| Skill | Steps (findings-cli.js subcommand in brackets) |
|---|---|
| `/bajzi:implement <slice>` | debt cap [`check`, exit 1 refuses the slice] → agent lookup from the slice's `tier:` [`slice`: 1 → `implementer-risk`, 2/3 → `implementer`] → dispatch the slice file verbatim → `BLOCKED` stops; `DONE` with a path outside `files:` stops → test → commit by path → prints the range |
| `/bajzi:review <slice> <range> [--round 2 --previous <file>]` | diff into the brief → `reviewer` → writes its message to `runtime/findings/<slice>-r<n>.md` [`validate`: verdict + open counts; one re-dispatch on a format error] → round 2 only: [`close`] D4 routing and the D6 cap line |
| `/bajzi:fix <r1-file>` | [`copy fixer`: the `.fixer.md`; a round-2 file exits 3] → `fixer` → report saved as `<slice>-r1.report.md` → gate (`.githooks/pre-commit`, else the slice test) → commit by path → `/bajzi:review --round 2` → **stop** |
| `/bajzi:debt --check\|--drain\|--calibrate` | `--check` [`check`, exit 1 on cap]; `--drain` [`copy fixer` on `debt.md`] → `fixer` → gate → commit → one `reviewer` round 2 over the drain range as slice `debt` [`drain`: drops resolved, prints `drained <n>, remain <m>` + remaining `if_unfixed`]; `--calibrate` [`copy blind`] → `reviewer` calibration mode [`calibrate`: `blind_severity` into `debt.md`, only disagreements to `needs-owner.md`] |
...
- `needs-owner.md` (`findings-cli.js:toOwner`): title `# Needs owner`, one appended line per
  ...
  `--calibrate`. An identical line is never appended twice, so re-running `close` is harmless.
- Exit codes: 0 ok, 1 refused (`DEBT CAP HIT`, a `debt.md` merge over 24 KB), 2 bad input
  (invalid file, missing round-1/round-2/report file, bad slice), 3 `STOP` (round cap).
- The skills commit (by path, never `-A`); the agents never do. The dispatch `description`
  ...
  guard classifies on, and the review brief carries the code-review-graph marker line (R1).
...
| `<cwd>/runtime/dispatch-sizes.log` | `dispatch-guard.sh:155-160` (§6.4); `findings-cli.js:cmds.log` (class `SKILL-<CLASS>`, §6.12) | the owner | TSV `<ISO-UTC> <class> <subagent_type> <prompt chars> <allow\|deny:R1\|R2\|R3>` (`dispatch-guard.sh:44-46`) | no cap; a failed write never changes the decision |
...
| `<cwd>/runtime/briefs/<slice>-<class>.txt` | the skills (`bajzi/skills/lib/dispatch.md` step 1) | `findings-cli.js log` (char count), the owner | plain text, the exact dispatch prompt | overwritten per dispatch of that class |
...
| Review/fix loop skills (§6.12, T5 of the agents-and-cadence plan) | `agents-cadence` branch (unreleased): `bajzi/skills/{implement,review,fix,debt}/SKILL.md`, `bajzi/skills/lib/dispatch.md`, `bajzi/lib/findings-cli.js`, `mode.sh` case 16 | no | a live end-to-end run (T9); dispatch-guard rewrite (T6); gate `.githooks/pre-commit` (T7); release 1.9.0 (T8) | `timeout 300 bash bajzi/skills/mode/tests/mode.sh </dev/null` → `PASS 205/205` |
