...
|---|---|---|---|
| bajzi node hook tests | `node --test bajzi/hooks/node/tests/*.test.js bajzi/skills/*/tests/*.test.js bajzi/agents/tests/*.test.js` | `bajzi-plugins-dev` | Git Bash or PowerShell (Node ≥18 expands the glob itself either way) |
...
| Agent scaffold + harness (§4.1, T0/T1 of the agents-and-cadence plan) | `agents-cadence` branch (unreleased): DAY-RUN-RULES.md Appendix A (T0); `bajzi/agents/` dir + `agents.test.js` contract + fixtures, `manifest.json` `plugins[].why` (T1) | no | agent bodies — `reviewer`/`fixer` (T3), `implementer`/`implementer-risk` (T4); findings format (T2); skills (T5); dispatch-guard rewrite (T6); gate (T7); release 1.9.0 (T8) | `node --test bajzi/agents/tests/agents.test.js` → `# fail 0`; `bash bajzi/skills/mode/tests/mode.sh` → `PASS 169/169` |
