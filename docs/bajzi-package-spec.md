...
| Findings format, severity rubric, fixer/blind copies, D4 close policy, D6 debt cap | `docs/findings-format.md` (the contract), `bajzi/lib/findings.js:parse`/`validate`/`stripForFixer`/`stripSeverity`/`applyClosePolicy`/`mergeToDebt`/`debtCapHit` | `node --test bajzi/lib/tests/findings.test.js` | 1 | It decides what reaches the owner, what is parked in `debt.md` and what the fixer sees. Every emitting/routing function validates first and throws on an invalid file; `debtCapHit` fails closed (an unparseable `debt.md` is a hit). A new round-2 finding is recognised only when `applyClosePolicy` gets the round-1 file as its third argument; without it every open finding counts as previously attempted and goes to the owner escalated. Keep `docs/findings-format.md` and the module in step: the doc's close-policy table mirrors `applyClosePolicy` branch for branch. |
