...
13. **The fixer never sees severity, and the reviewer never writes.** `fixer` is dispatched on a
    ...
    `/bajzi:fix`, which stop after round 2: once a slice has its `-r2.md`, `findings-cli.js`
...
- **R2** — every class but `FIXER` and `REVIEWER`: deny, "fixed through `/bajzi:fix`", if the
  ...
  `runtime/briefs/*.diff` is never an R2 path).
...
- Exit codes: 0 ok, 1 an unexpected error (a Node crash; treat as STOP), 2 bad input (invalid
  file, missing round-1/round-2/report file, bad slice, a slice id or class outside
...
| Dispatch guard (§6.4) | `env-unify` (merged from `saver-levels` @ `809bc18`); rewritten on `agents-cadence` (T6: typed classification, R1/R2 per the plan's R1'/R2', R3 on every non-fixer dispatch) | **no** | release 1.8.0, then 1.9.0 (T8) | `git merge-base --is-ancestor 809bc18 env-unify` → exit 0; `grep -c dispatch-guard …/1.7.0/hooks/hooks.json` → `0` |
...
| Agent scaffold + harness (§4.1, T0/T1 of the agents-and-cadence plan) | `agents-cadence` branch (unreleased): DAY-RUN-RULES.md Appendix A (T0); `bajzi/agents/` dir + `agents.test.js` contract + fixtures, `manifest.json` `plugins[].why` (T1) | no | release 1.9.0 (T8) | `node --test bajzi/tests/agents/agents.test.js` → `# pass 19`, `# fail 0` |
