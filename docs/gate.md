...
| `tsc --noEmit --pretty false` | project-wide, per `package.json` dir that also has `tsconfig.json` | that dir exists | `error TSnnnn:` lines summed; block if > baseline `tsc` |
...
- **Absent stack** (no marker file) = skipped silently, never a block. **Needed but missing** =
...
- A non-zero ratchet run with no readable count (unparsable JSON, `tsc` with no `error TS` line)
  is a tool error, never a pass.
...
- It lints the working-tree copy of each staged file; a partially staged file is judged on its
